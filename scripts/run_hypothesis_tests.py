...
from src.hypothesis_testing import calculate_risk_differences, calculate_margin_differences
...
def run_tests():
    ...
    # Frequency
    res_prov_freq = calculate_risk_differences(df, 'Province', 'TotalClaims', 'frequency')
    ...
    # Severity
    res_prov_sev = calculate_risk_differences(df, 'Province', 'TotalClaims', 'severity')
    ...
    # Frequency
    res_zip_freq = calculate_risk_differences(df_zip, 'PostalCode', 'TotalClaims', 'frequency')
    print(f"   [Frequency] p-value: {res_zip_freq['P-Value']:.4e} | Significant: {res_zip_freq['Significant']}")
    
    # Severity
    res_zip_sev = calculate_risk_differences(df_zip, 'PostalCode', 'TotalClaims', 'severity')
    ...
    res_zip_margin = calculate_margin_differences(df_zip, 'PostalCode')
    ...
    if 'Gender' in df.columns:
        ...
        if not df_gender.empty:
            res_gen_freq = calculate_risk_differences(df_gender, 'Gender', 'TotalClaims', 'frequency')
            print(f"   [Frequency] p-value: {res_gen_freq['P-Value']:.4e} | Significant: {res_gen_freq['Significant']}")
            
            res_gen_sev = calculate_risk_differences(df_gender, 'Gender', 'TotalClaims', 'severity')
